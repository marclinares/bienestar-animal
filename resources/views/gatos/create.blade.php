...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-serif font-bold text-[#4A2810]">Nuevo Registro Felino</h1>
            <p class="text-[#7A4F2D]/70 font-medium italic">Expediente oficial de identificación de colonia</p>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-xl border border-[#F0DEC4] overflow-hidden">
        <div class="bg-[#7A4F2D] px-8 py-4 flex items-center gap-3">
            ...
            <span class="text-xs font-bold uppercase tracking-widest text-[#F5E6D3]">Complete todos los campos requeridos para el censo</span>
        </div>

        <form action="{{ route('gatos.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-10">
            @csrf

            <div class="space-y-6">
                <div class="flex items-center gap-2 border-b border-[#F0DEC4] pb-2">
                    <span class="text-[#D94F3D] font-serif italic text-xl">01.</span>
                    <h3 class="text-sm font-bold uppercase tracking-widest text-[#4A2810]">Identificación y Ubicación</h3>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Nombre del Gato <span class="text-[#D94F3D]">*</span></label>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Colonia de Origen <span class="text-[#D94F3D]">*</span></label>
                        <select name="colonia_id" required class="block w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D] appearance-none cursor-pointer">
                            @foreach($colonias as $colonia)
                                <option value="{{ $colonia->id }}" {{ (isset($colonia_id) && $colonia_id == $colonia->id) || old('colonia_id') == $colonia->id ? 'selected' : '' }}>
                                    {{ $colonia->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Raza o Capa Visual</label>
                        <input type="text" name="raza" value="{{ old('raza') }}" class="block w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810]" placeholder="Ej: Atigrado, carey, naranja...">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Edad estimada</label>
                            <input type="number" name="edad" value="{{ old('edad') }}" class="block w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810]" placeholder="Años">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Estado <span class="text-[#D94F3D]">*</span></label>
                            <select name="estado" class="block w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810]">
            ...
            <div class="space-y-4">
                <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Descripción y Notas Médicas</label>
                <textarea name="descripcion" rows="3" class="block w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D]" placeholder="Indique marcas distintivas, carácter o tratamientos en curso...">{{ old('descripcion') }}</textarea>
            </div>
