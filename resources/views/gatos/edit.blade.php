...
<div class="max-w-4xl mx-auto py-8 px-4">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <a href="{{ route('colonias.show', $gato->colonia_id) }}" class="text-[#7A4F2D] text-sm font-bold flex items-center gap-1 hover:text-[#D94F3D] mb-2 transition-colors">
                ← Volver a la colonia
            </a>
            <h1 class="text-3xl font-serif font-bold text-[#4A2810]">Actualizar Registro</h1>
            <p class="text-[#7A4F2D]/60 text-xs font-bold uppercase tracking-widest">ID Felino: #GA-{{ $gato->id }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-xl border border-[#F0DEC4] overflow-hidden">
        <form action="{{ route('gatos.update', $gato) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-10">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div class="flex items-center gap-2 border-b border-[#F0DEC4] pb-2">
                    ...
                    <h3 class="text-sm font-bold uppercase tracking-widest text-[#4A2810]">Información del Felino</h3>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Nombre</label>
                        <input type="text" name="nombre" value="{{ old('nombre', $gato->nombre) }}" required
                            class="w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D] focus:border-[#D94F3D]">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Colonia Asignada</label>
                        <select name="colonia_id" class="w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D] focus:border-[#D94F3D]">
                            @foreach($colonias as $colonia)
                                <option value="{{ $colonia->id }}" {{ old('colonia_id', $gato->colonia_id) == $colonia->id ? 'selected' : '' }}>
                                    {{ $colonia->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Raza o Capa</label>
                        <input type="text" name="raza" value="{{ old('raza', $gato->raza) }}"
                            class="w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D] placeholder-[#7A4F2D]/30" placeholder="Ej: Común europeo">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="block text-xs font-bold uppercase tracking-widest text-[#7A4F2D] ml-1">Edad aprox.</label>
                            <input type="number" name="edad" value="{{ old('edad', $gato->edad) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="estado" class="w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D]">
                                <option value="disponible" {{ old('estado', $gato->estado) == 'disponible' ? 'selected' : '' }}>Disponible</option>
                                <option value="adoptado"   {{ old('estado', $gato->estado) == 'adoptado'   ? 'selected' : '' }}>Adoptado</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea name="descripcion" rows="3"
                            class="w-full rounded-2xl border-[#F0DEC4] bg-[#F5E6D3]/10 px-4 py-3 text-[#4A2810] focus:ring-[#D94F3D]">{{ old('descripcion', $gato->descripcion) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-8 border-t border-[#F0DEC4] space-y-6">
                <div class="flex items-center justify-between border-b border-[#F0DEC4] pb-2">
                    ...
                    <span id="contador-fotos" class="text-xs font-bold text-[#7A4F2D] bg-[#F5E6D3] px-3 py-1 rounded-full">
                        {{ $gato->fotos->count() }}/3 fotos
                    </span>
                </div>

                <p class="text-[10px] text-[#7A4F2D]/60 uppercase font-bold tracking-tighter">
                    Marca el icono 🗑 para eliminar una foto al guardar. Puedes subir hasta {{ 3 - $gato->fotos->count() }} foto(s) nueva(s).
                </p>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4" id="grid-fotos">
                    ...
                    <div class="relative group aspect-square" id="foto-actual-{{ $foto->id }}">
                        ...
                        <div class="absolute bottom-2 left-2 pointer-events-none">
                            <span class="text-[8px] font-bold text-white uppercase bg-black/40 px-2 py-0.5 rounded-md backdrop-blur-sm shadow-sm">Subida</span>
                        </div>
                    </div>
                    @endforeach
                    ...
                    <label id="uploader-label" class="relative flex flex-col items-center justify-center w-full py-8 rounded-2xl border-2 border-dashed border-[#F0DEC4] bg-[#F5E6D3]/5 hover:bg-[#F5E6D3]/20 transition-all cursor-pointer group">
                        ...
                        <div class="flex flex-col items-center gap-2">
                            ...
                            <p class="text-[9px] text-[#7A4F2D]/50 italic" id="huecos-label">
                                {{ 3 - $gato->fotos->count() }} hueco(s) disponible(s) · Máximo 3 fotos totales
                            </p>
                        </div>
                    </label>
                </div>
            ...
            <div class="flex items-center justify-end gap-6 pt-10 border-t border-[#F0DEC4]">
                <a href="{{ route('colonias.show', $gato->colonia_id) }}"
...
<script>
const totalActuales = {{ $gato->fotos->count() }};
